...
const FinancialOverview = () => {
  ...
  const [revenueData, setRevenueData] = useState<Array<{year: string; revenue: number; profit: number}>>([]);
  ...
  useEffect(() => {
    fetch(`${API_URL}/revenue-trends`)
      ...
      .then((data) => {
        ...
        const chartData = data.years.map((year: number, i: number) => ({
          year: year.toString(),
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="bg-slate-800/50 border-slate-700">
        <CardHeader>
          <CardTitle className="text-white">Annual Revenue Trends</CardTitle>
          <CardDescription className="text-slate-300">
            Revenue and profit over the past years ($ Millions)
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            <div>Loading revenue trends...</div>
          ) : error ? (
